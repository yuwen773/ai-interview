    ...
    <div className="bg-[var(--color-bg)] dark:bg-[var(--color-bg-dark)] rounded-xl p-4">
      ...
      <div className="w-full aspect-square max-w-[120px] mx-auto mb-3 relative">
        <svg viewBox="0 0 100 100" className="w-full h-full">
          ...
          <polygon points="50,18 82,37 78,68 50,85 22,65 20,37" fill="var(--color-primary)" fillOpacity="0.15" stroke="var(--color-primary)" strokeWidth="1.5" />
        </svg>
      </div>
      <div className="grid grid-cols-2 gap-x-3 gap-y-1">
        {MOCK_REPORT_CATEGORIES.slice(0, 4).map((cat) => (
          <div key={cat.name} className="flex items-center justify-between text-[10px]">
            ...
            <span className="font-medium text-[var(--color-text)] dark:text-[var(--color-text-dark)]">{cat.score}/10</span>
          </div>
        ))}
      </div>
    </div>
...
function MockProfileUI() {
  return (
    <div className="bg-[var(--color-bg)] dark:bg-[var(--color-bg-dark)] rounded-xl p-4 space-y-3">
      ...
      {MOCK_PROFILE_TOPICS.map((t) => {
        const z = ZONE_COLORS[t.zone];
        return (
...
function FeatureCard({ feature, index, theme }: { feature: FeatureItem; index: number; theme: 'light' | 'dark' }) {
  ...
  return (
    ...
    >
      <BorderGlow
        ...
        fillOpacity={0.4}
      >
        <div className={`grid grid-cols-1 md:grid-cols-2 gap-0 overflow-hidden rounded-2xl ${isReversed ? 'md:[direction:rtl]' : ''}`}>
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 gap-6">
          {FEATURES.map((feature, i) => (
            <FeatureCard key={feature.title} feature={feature} index={i} theme={theme} />
          ))}
        </div>
      </div>
    </section>
