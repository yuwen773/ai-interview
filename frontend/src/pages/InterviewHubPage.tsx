...
export default function InterviewHubPage() {
  ...
  return (
    <div className="max-w-5xl mx-auto">
      ...
      {/* 配置区域 */}
      <div className="bg-white dark:bg-[var(--color-surface-dark)] rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 p-6 mb-8">
        <div className="space-y-6">
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-3 p-4 rounded-xl border-2 transition-all duration-200 text-left
                      ${selected
                        ? 'border-[var(--color-primary)] bg-[var(--color-primary-subtle)] dark:bg-[var(--color-primary-subtle-dark)]'
                        : 'border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 hover:border-slate-300 dark:hover:border-slate-600'
                      }`}
                  >
                    <Icon className={`w-6 h-6 flex-shrink-0 ${selected ? 'text-[var(--color-primary)]' : 'text-slate-400'}`} />
                    <div className="min-w-0">
                      <p className={`font-semibold text-sm flex items-center gap-2 ${selected ? 'text-[var(--color-primary-hover)] dark:text-[var(--color-primary)]' : 'text-slate-900 dark:text-white'}`}>
                        <span>{opt.label}</span>
                        {opt.recommended && (
                  ...
                  return (
                    ...
                    >
                      <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-sm flex-shrink-0 ${
                        selected ? skill.display?.iconBg || 'bg-amber-100 dark:bg-amber-900/50' : 'bg-slate-100 dark:bg-slate-700'
                      }`}>
                        {fallbackEmoji}
                      </div>
                      <div className="flex-1 min-w-0">
                        <span className={`text-xs font-medium block truncate ${selected ? 'text-[var(--color-primary-hover)] dark:text-[var(--color-primary)]' : 'text-slate-700 dark:text-slate-300'}`}>
                          {skill.name}
                        </span>
                      </div>
                    </button>
                ...
                <button
                  ...
                  className={`flex items-center gap-2.5 p-3 rounded-xl border-2 border-dashed transition-all duration-200 text-left
                    ${config.isCustomSkill
                      ? 'border-[var(--color-primary)] bg-[var(--color-primary-subtle)] dark:bg-[var(--color-primary-subtle-dark)]'
                      : 'border-slate-200 dark:border-slate-700 hover:border-[var(--color-primary)] dark:hover:border-[var(--color-primary)]'
                    }`}
                >
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${
                    config.isCustomSkill ? 'bg-amber-100 dark:bg-amber-900/50' : 'bg-slate-100 dark:bg-slate-700'
                  }`}>
                    <span className="text-sm">✨</span>
                  </div>
                  <span className={`text-xs font-medium ${config.isCustomSkill ? 'text-[var(--color-primary-hover)] dark:text-[var(--color-primary)]' : 'text-slate-500 dark:text-slate-400'}`}>
                    自定义 JD
                  </span>
                </button>
              </div>
                  ...
                  {config.customCategories.length > 0 && (
                    <div className="flex flex-wrap gap-2">
                      {config.customCategories.map((cat, i) => (
                        ...
                        >
                          {cat.label}
                          <span className="ml-1 text-[10px] text-amber-500">({cat.priority})</span>
                        </span>
                      ))}
                    </div>
                  )}
            ...
            <div className="grid grid-cols-3 gap-3">
              {DIFFICULTY_OPTIONS.map(opt => {
                ...
                return (
                  ...
                  >
                    <p className={`text-sm font-semibold ${selected ? 'text-[var(--color-primary-hover)] dark:text-[var(--color-primary)]' : 'text-slate-700 dark:text-slate-300'}`}>
                      {opt.label}
                    </p>
          ...
          <AnimatePresence>
            {config.showMore && (
              ...
              >
                {/* 简历选择 */}
                <div className="bg-gradient-to-br from-[var(--color-primary-subtle)] to-blue-50/80 dark:from-[var(--color-primary-subtle-dark)] dark:to-blue-900/10 rounded-xl p-4 border border-amber-100 dark:border-amber-800/30">
                  ...
                  <select
                    ...
                    className="w-full px-4 py-2.5 rounded-lg border border-amber-200 dark:border-amber-700/50
                      bg-white dark:bg-slate-800 text-sm text-slate-900 dark:text-white
                ...
                {config.mode === 'voice' && (
                  <div className="bg-slate-50/80 dark:bg-slate-900/50 rounded-xl p-4 border border-slate-200 dark:border-slate-700">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <div className="text-2xl font-bold tabular-nums text-[var(--color-primary)] dark:text-[var(--color-primary)]">
                    ...
                    <input
                      ...
                      className="w-full h-2 bg-slate-200 dark:bg-slate-700 rounded-lg appearance-none cursor-pointer
                        ...
                        [&::-webkit-slider-thumb]:bg-[var(--color-primary)] [&::-webkit-slider-thumb]:cursor-pointer
                        [&::-webkit-slider-thumb]:shadow-md [&::-webkit-slider-thumb]:shadow-[var(--color-primary)]/30"
                    />
                  </div>
                )}
              </motion.div>
        ...
        <div className="mt-6 pt-6 border-t border-slate-100 dark:border-slate-700">
          <motion.button
            ...
            className="w-full px-6 py-3 rounded-xl font-semibold text-sm transition-all
              ...
              text-white shadow-lg shadow-[var(--color-primary)]/25 disabled:opacity-50 disabled:cursor-not-allowed"
          >
      ...
      {/* 最近面试记录 */}
      <div className="bg-white dark:bg-[var(--color-surface-dark)] rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 p-6">
            ...
            {recentInterviews.map((item, index) => {
              ...
              return (
                ...
                >
                  ...
                  {/* 箭头 */}
                  <svg className="w-4 h-4 text-slate-300 dark:text-slate-600 group-hover:text-[var(--color-primary)] group-hover:translate-x-0.5 transition-all flex-shrink-0" viewBox="0 0 24 24" fill="none">
