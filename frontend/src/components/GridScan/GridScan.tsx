...
function smoothDampVec2(
  ...
  const adjTarget = current.clone().sub(change);
  const temp = currentVelocity.clone().addScaledVector(change, omega).multiplyScalar(deltaTime);
  currentVelocity.sub(temp.clone().multiplyScalar(omega));
  currentVelocity.multiplyScalar(exp);

  out.copy(adjTarget.clone().add(change.add(temp).multiplyScalar(exp)));
...
export default function GridScan({
  ...
  const handleVisibility = useCallback<IntersectionObserverCallback>((entries) => {
  ...
  useEffect(() => {
    ...
    const onClick = async () => {
      ...
      if (
        ...
        ((window as unknown as { DeviceOrientationEvent: { requestPermission?: () => Promise<string> } }).DeviceOrientationEvent).requestPermission
      ) {
        try {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      // skip heavy rendering when scrolled out of view
  ...
  // uniform sync (non-reconstructive updates)
  useEffect(() => {
    ...
    const start = async () => {
      ...
      const detect = async (ts: number) => {
        ...
        if ('requestVideoFrameCallback' in HTMLVideoElement.prototype) {
          ...
          requestAnimationFrame(() => detect(performance.now()));
        }
      };
